...
import type { EntradaResponse, MovementsResult, Movement, StockItem } from '$lib/types/stock';
import { buscarItem } from '$lib/api/stock/items';
import { listarEntradasPorItem } from '$lib/api/stock/movements';

export const ssr = false;
export const prerender = false;

interface EntryLine extends Movement {
	id: string;
}

function toM(e: EntradaResponse, unit: string): EntryLine {
	return {
		id: e.id,
		type: 'in',
		kind: e.idFornecedor ? 'compra' : undefined,
		item: { id: e.idItem, name: e.nomeItem ?? e.idItem, unit },
		quantity: e.quantidade,
		date: e.dataEntrada,
		origin: e.fornecedor?.nome ?? '—',
		reference: e.notaFiscal ?? undefined,
		unitValue: e.valorUnitario,
...
function matches(e: EntryLine, term: string): boolean {
	const t = normalize(term);
	if (!t) return true;
	return (
		normalize(e.item.name).includes(t) ||
		normalize(e.origin ?? '').includes(t) ||
...
export const load: PageLoad = async ({ url, fetch }) => {
	...
	try {
		const [item, entradas] = await Promise.all([
			buscarItem(idItem, fetch),
			listarEntradasPorItem(idItem, fetch)
		]);

		let lines = entradas.map((e) => toM(e, item.unidadeMedida));
		...
		return {
			...
			error: err instanceof Error ? err.message : 'Não foi possível carregar as entradas'
		};
	}
};